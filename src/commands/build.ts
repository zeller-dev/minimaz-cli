...
import {
    ...
    // --- TYPES ---
    Bundle, Bundles, File, MinimazConfig
} from '../index.js'
...
export async function build(): Promise<void> {
    ...
    const outDirPath: string =
        path.resolve(resolveCurrentPath(), config.outDir)
    ...
    {
        outDir: config.bundling?.outDir
            ? path.join(outDirPath, config.bundling.outDir)
            : outDirPath,
        css: {
            outFile: config.bundling?.outDir
                ? path.join(outDirPath, config.bundling.outDir)
                : outDirPath,
            chunks: []
        },
 ...
 * @param config - Minimaz configuration
 * @param bundles: css and js bundles
 */
    ...
    for (
        ...
        if (fileContent.length > 0) {
            const fileExt: string =
                path.extname(fromPath).toLowerCase()
...
async function processHtml(
    ...
    const scriptRegex: RegExp = /<script([^>]*)>([\s\S]*?)<\/script>/gi
    let result: string = file.content // Start with original content

    // 1. Minify Inline Scripts
    const matches: RegExpExecArray[] =
        Array.from(result.matchAll(scriptRegex))
    ...
    for (
        ...
        const typeAttr: string = (
            attrs.match(/type=["']([^"']+)["']/i)?.[1] || ''
        ).toLowerCase()
        const isModule: boolean = /module/i.test(typeAttr)
        ...
        if (typeAttr === '' || typeAttr === 'text/javascript' || isModule) {
            try {
                ...
                result = result.replace(
                    fullMatch,
                    `<script${attrs}>${minified.trim()}</script>`
                )
            } catch (error: any) {
...
async function processCSS(
    ...
    if (bundling) {
        bundle.chunks.push(file.content)
    } else {
...
async function processTS(
    ...
    if (/require\s*\(|module\.exports|exports\./.test(file.content))
        log(
            'warn',
            `CommonJS detected in ${file.src}, consider converting to ESM`
        )
...
async function writeCssBundle(
    ...
    log('success', `CSS bundle written to ${outDir}/${outFile}`)
...
async function writeJsBundle(
    ...
    try {
        ...
        log('success', `JS bundle written to ${outDir}/${outFile}`)
    } catch (error: any) {
        log('warn', `ESBuild JS bundle failed: ${error}`)
    }
}